...
int WriteTree2String(struct TreeInfo *info, struct Node *n, string& sfile, int newlines);
...
int WriteChar(FILE *sfile, char c, int spc)
{
	...
	if(spc && isspace(temp) && (save_linelen >= MAXTEXT_LINELEN))
		c = '\n';
...
int WritePropValue2string(char *v, int second, U_SHORT flags, string& sfile)
{
	U_SHORT fl;
...
int WriteProperty2string(struct TreeInfo *info, struct Property *prop, string& sfile)
{
	static int gi_written = FALSE;
	...
	while(v)
	{
		...
		if(do_tt && !strlen(v->value))
			WritePropValue2string("tt", FALSE, prop->flags, sfile);
		else
		{
			WritePropValue2string(v->value, FALSE, prop->flags, sfile);
			WritePropValue2string(v->value2, TRUE, prop->flags, sfile);
		}
		sfile += ']';
...
int WriteNode2string(struct TreeInfo *info, struct Node *n, string& sfile)
{
	struct Property *p;
	save_chars_in_node = 0;
	save_eol_in_node = 0;
	...
	while(p)
	{
		...
		if(!WriteProperty2string(info, p, sfile))
			return(FALSE);
...
int WriteTree2String(struct TreeInfo *info, struct Node *n, string& sfile, int newlines)
{
	...
	if(!WriteNode2string(info, n, sfile))
		return(FALSE);
	...
	while(n)
	{
		if(n->sibling)
		{
			while(n)					/* write child + variations */
			{
				if(!WriteTree2String(info, n, sfile, 1))
					return(FALSE);
				n = n->sibling;
			}
		}
		...
		{
			if(!WriteNode2string(info, n, sfile))	/* write child */
				return(FALSE);
			n = n->child;
		}
	}
...
void SaveSGF2string(struct SGFInfo *sgf, string& sfile)
{
	...
	while(n)
	{
		if(!WriteTree2String(info, n, sfile, nl))
		{
